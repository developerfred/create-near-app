...
if (![undefined, 'react', 'vanilla', 'vue'].includes(process.env.FRONTEND)) {
    console.error(
        `Unknown FRONTEND template '${process.env.FRONTEND}'; ` +
        'use \'react\' or \'vanilla\' or \'vue\''
    )
    process.exit(1)
}

const contracts = process.env.CONTRACT ? [process.env.CONTRACT] : ['assemblyscript', 'rust']

const frontends = process.env.FRONTEND ? [process.env.FRONTEND] : ['vanilla', 'react', 'vue']
